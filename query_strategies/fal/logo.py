...
class LoGo(Strategy):
    def query(self, user_idx, label_idxs, unlabel_idxs, n_query=100):
        ...
        g_net = self.net
        l_net = self.training_local_only(user_idx, label_idxs)
        
        # cluster with uncertain samples by local net
        embedding = self.get_grad_embedding_maxInd(user_idx, list(unlabel_idxs), net=l_net)
        ...
        for c_i in cluster_dict.keys():
            ...
            probs = self.predict_prob(user_idx, cluster_idxs, g_net)
