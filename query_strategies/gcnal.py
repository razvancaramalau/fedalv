...
from .badge_sampling import init_centers
...
class GCNAL2(Strategy):
    def __init__(self, dataset_query, dataset_train, net, args):
        super(GCNAL2, self).__init__(dataset_query, dataset_train, net, args)
    ...
    def query(self, user_idx, label_idxs, unlabel_idxs, n_query=100):        
        ...
        net_local = self.training_local_only(label_idxs)

        embeds = self.get_embedding(data_idxs, net)
        embeds_local = self.get_embedding(data_idxs, net_local)
        
        u_features = embeds_local[:len(random_unlabel_idxs), :]
        l_features = embeds[len(random_unlabel_idxs):, :]
...
class GCNAL3(Strategy):
    def __init__(self, dataset_query, dataset_train, net, args):
        super(GCNAL3, self).__init__(dataset_query, dataset_train, net, args)
    ...
    def query(self, user_idx, label_idxs, unlabel_idxs, n_query=100):        
        ...
        if self.args.query_model_mode == "global":
            gradEmbedding = self.get_grad_embedding(data_idxs, net=self.net)
        elif self.args.query_model_mode == "local_only":
            net = self.training_local_only(label_idxs)
            gradEmbedding = self.get_grad_embedding(data_idxs, net=net)
        
        # embeds = self.get_embedding(data_idxs, net)
        
        u_features = gradEmbedding[:len(random_unlabel_idxs), :]
        l_features = gradEmbedding[len(random_unlabel_idxs):, :]
        ...
        with torch.no_grad():
            ...
            if self.method == "CoreGCN":
                feat = feat.detach().cpu().numpy()
                chosen = init_centers(feat[nlbl, :], n_query)
